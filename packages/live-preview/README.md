...
build a render plan when they need one. If you pass `initialPayload`,
`getSnapshot()` returns it until a valid update arrives. Omitting
`initialPayload` starts the snapshot at `undefined`. Each `subscribe` call returns
...
`PreviewSessionOptions`. `getSnapshot()` exposes the latest Preview Session data to
the application.
