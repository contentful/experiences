...
```
experiences/
├── nx.json                   # Nx workspace (target defaults, plugins). Release config lives per-package.
...
Conventional commits (`feat:`, `fix:`, `chore:`, `docs:`, etc.). Enforced via `commitlint.config.js` + `.husky/commit-msg`. **`feat:` and `fix:` trigger version bumps via `semantic-release`. `chore:` does NOT trigger a release.** During alpha, breaking changes (`feat!:` or `BREAKING CHANGE:`) are remapped to a minor bump so packages stay under 1.0.0 — see the release-rule override in each package's `package.json` `release.plugins` block. Remove this override when moving to 1.0.0 GA.
...
4. Update `package.json#version` → `"0.0.0-determined-by-semantic-release"` and `release.tagFormat` → `"adapter-vue@${version}"`
...
8. Create a stub `packages/adapter-vue/CHANGELOG.md` with just `# Changelog\n`. `@semantic-release/changelog` prepends to any existing file — without a stub, it fails to write.
9. **Seed the tag** before the first release: `git tag adapter-vue@0.0.0 && git push origin adapter-vue@0.0.0`. Without a seed tag, semantic-release's first run would jump straight to 1.0.0. The seed keeps new packages starting at `0.0.x` / `0.1.0` consistent with existing packages.
10. Add the new package dir to the release loop in `.github/workflows/release.yaml`. Adapters go **after** internal packages in the topological order.
11. Coordinate the OIDC trusted-publisher setup on npmjs.org for the new package name before the first CI release.
...
Releases are fully automated by CI on push to `main`. Every `feat:` / `fix:` / `feat!:` / `build(deps):` commit that touched a package's directory triggers a version bump for that package on the next merge to `main`. `chore:` and `docs:` do NOT release.

Local dry-run to preview what a package would release without publishing:
