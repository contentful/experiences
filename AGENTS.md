...
`<svelte:boundary onerror failed>` does not run its catching machinery under `svelte/server` — verified with an isolated repro (a bare boundary + a synchronously-throwing child, no adapter code involved) before trusting it, because this is easy to get backwards. A throw during SvelteKit's server render propagates and fails the _entire_ page render; the boundary only catches client-side. React and Angular don't share this gap (React's per-node boundary wraps its content in an internal `<Suspense>`, which Fizz _does_ honor for a thrown Error during streaming SSR, and even the legacy `renderToString`/`renderToStaticMarkup` APIs turned out to honor it too — undocumented React behavior, also verified empirically; Angular has no separate server renderer at all). See the README's [Error handling & troubleshooting](./README.md#error-handling--troubleshooting) section for the full per-framework table. No fix exists at the adapter level as of Svelte 5.56 — this is upstream Svelte behavior (`svelte/server` exposes no per-node recovery primitive below its single top-level `render()` call). Two candidate code-level fixes were prototyped for AIS-364 and rejected: manually splicing a per-node `render()` call in via `{@html}` permanently breaks hydration for that node (no hydration markers survive it), and a speculative "probe render" before the real one silently double-invokes every customer component's construction/lifecycle on every request. The README ships a `handleError`-hook-based mitigation instead (turns the crash into a controlled response, doesn't give per-node isolation). Revisit if/when Svelte ships SSR-aware error boundaries.
...
`NodeRenderEngine.createView` wraps `viewContainerRef.createComponent(...)` in try/catch, which catches a throw from a component's constructor, template evaluation, or `ngOnInit` — everything Angular runs synchronously during creation. `collect()` separately wraps its own `unit.resolution()` read — a throw from `resolveNode`/`resolveDesign` recomputing on a _later_ sync (a customer-supplied `resolveToken` that starts throwing, say) now swaps that node to the error fallback the same way, and recovers automatically if resolution succeeds again on a later sync (tracked via `unit.resolutionFailed`, mirroring the existing `attemptedComponentType` "don't retry the same failure every sync" pattern). This was the right fix location, not `bindings()`'s individual `inputBinding` getters (the original hypothesis) — traced the actual signal wiring and confirmed `resolution`'s only dependencies (`unit.node`, `experienceScope.config()`) are exclusively mutated from inside `collect()` itself, so any throw from a real dependency change always reaches `collect()`'s own read first; a `bindings()`-level catch would have been unreachable dead code.

What's still NOT caught: a throw inside the _customer_ component's own internals on a later change-detection pass (its own template expression, computed, or lifecycle hook unrelated to our bindings) — that never touches adapter code at all, so nothing here is positioned to catch it. A per-node `ErrorHandler` provider in the unit's injector looked like the natural fix for that case, since providers already flow per-node in this adapter, but was verified NOT to work: `ApplicationRef.tick()` resolves `ErrorHandler` once, from the _root_ injector, at construction, and never re-resolves it from whichever component's injector actually threw — a per-node override in a child injector is structurally unreachable from that catch, regardless of zoneless vs. zone-based change detection (isolated repro: a component whose value getter starts throwing on a later `detectChanges()`, with a custom `ErrorHandler` provided in its creation injector — never invoked). Documented as a residual gap rather than shipped as a misleading full fix. Revisit if Angular's error-handling internals change, or if someone finds the right injection point (candidate: hooking `ApplicationRef`'s zone-level error handling instead of DI).
