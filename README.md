...
- **Angular**: **partially caught.** `NodeRenderEngine.collect()` now guards the *resolution* recomputation itself (`resolveNode`/`resolveDesign` — e.g. a customer-supplied `resolveToken` that starts throwing) — if that throws on a later sync, the affected node swaps to the error fallback the same way a creation-time throw does, siblings unaffected, and it recovers automatically if resolution succeeds again on a subsequent sync. **What's still not caught**: a throw inside the *customer* component's own internals on a later change-detection pass (its own template expression, computed, or lifecycle hook) — that never touches adapter code at all, so there's nothing in this adapter positioned to catch it. A per-node `ErrorHandler` provider was tried for that case — the natural-looking fix, since providers already flow per-node in this adapter — and **empirically does not get consulted**: `ApplicationRef.tick()` resolves `ErrorHandler` once, from the *root* injector, at construction, and never re-resolves it from whichever component's injector actually threw — a per-node override in a child injector is structurally unreachable from that catch, regardless of zoneless vs. zone-based change detection. Verified with an isolated repro, not assumed. Documented here as a known residual gap rather than shipped as a misleading full fix.
