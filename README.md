...
- If you render Svelte through SvelteKit's SSR, a throwing component **fails the whole page** server-side today. This is `<svelte:boundary>`'s own limitation (`svelte/server` exposes no per-node recovery primitive below its single top-level `render()` call — confirmed by reading its source, not assumed), not a bug in this SDK, and there is no adapter-level fix available in Svelte 5.56. **Mitigating this in SvelteKit**: add a [`handleError` hook](https://svelte.dev/docs/kit/hooks#Shared-hooks-handleError) in `src/hooks.server.ts`. It won't give you per-node isolation, but it turns an unhandled SSR crash into a controlled error response/page instead of a raw server error — the difference between "the whole page 500s with no context" and "the whole page 500s with a message you chose and a chance to log it." Test your registered components' happy _and_ unhappy paths before shipping regardless; don't rely on the SSR pass to catch a bad component for you.
...
- **Angular**: **partially caught.** `NodeRenderEngine.collect()` now guards the _resolution_ recomputation itself (`resolveNode`/`resolveDesign` — e.g. a customer-supplied `resolveToken` that starts throwing) — if that throws on a later sync, the affected node swaps to the error fallback the same way a creation-time throw does, siblings unaffected, and it recovers automatically if resolution succeeds again on a subsequent sync. **What's still not caught**: a throw inside the _customer_ component's own internals on a later change-detection pass (its own template expression, computed, or lifecycle hook) — that never touches adapter code at all, so there's nothing in this adapter positioned to catch it. A per-node `ErrorHandler` provider was tried for that case — the natural-looking fix, since providers already flow per-node in this adapter — and **empirically does not get consulted**: `ApplicationRef.tick()` resolves `ErrorHandler` once, from the _root_ injector, at construction, and never re-resolves it from whichever component's injector actually threw — a per-node override in a child injector is structurally unreachable from that catch, regardless of zoneless vs. zone-based change detection. Verified with an isolated repro, not assumed. Documented here as a known residual gap rather than shipped as a misleading full fix.
